...
class JOSE::JWK::PEMTest < Minitest::Test
  def test_from_pem_and_to_pem
    ...
    assert_equal OpenSSL::PKey::EC.new(ec_pem_data).to_pem.strip, JOSE::JWK.to_pem(ec_pem).strip

    ec_pem_password = SecureRandom.urlsafe_base64(16)
    encrypted_ec_pem_data = JOSE::JWK.to_pem(ec_pem, ec_pem_password)
    refute_equal OpenSSL::PKey::EC.new(ec_pem_data).to_pem.strip, encrypted_ec_pem_data.strip
    assert_equal ec_pem, JOSE::JWK.from_pem(encrypted_ec_pem_data, ec_pem_password)

    rsa_pem_data = <<~PEM.strip
    ...
    assert_equal OpenSSL::PKey::RSA.new(rsa_pem_data).to_pem.strip, JOSE::JWK.to_pem(rsa_pem).strip

    rsa_pem_password = SecureRandom.urlsafe_base64(16)
    encrypted_rsa_pem_data = JOSE::JWK.to_pem(rsa_pem, rsa_pem_password)
    refute_equal OpenSSL::PKey::RSA.new(rsa_pem_data).to_pem.strip, encrypted_rsa_pem_data.strip
    ...
    x509_key = OpenSSL::X509::Certificate.new(x509_pem_data.strip).public_key
